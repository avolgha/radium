...
impl Shell {
    ...
    #[allow(dead_code)]
    pub fn concise<F>(&mut self, mut callback: F)
    where
        F: FnMut(&mut Shell),
    {
        match self.verbosity {
    ...
    #[allow(dead_code)]
    pub fn set_color_choice(&mut self, color: Option<&str>) -> Result<(), Error> {
    ...
    #[allow(dead_code)]
    pub fn write_stdout(&mut self, fragment: impl fmt::Display, color: &ColorSpec) {
    ...
    /// Caller is responsible for deciding whether [`Shell::verbosity`] is affects output.
    #[allow(dead_code)]
    pub fn write_stderr(&mut self, fragment: impl fmt::Display, color: &ColorSpec) {
        self.output.write_stderr(fragment, color)
    }

    /// Prints a message to stderr and translates ANSI escape code into console colors.
    #[allow(dead_code)]
    pub fn print_ansi_stderr(&mut self, message: &[u8]) {
        if self.needs_clear {
            self.err_erase_line();
        }
        #[cfg(windows)]
    ...
    #[allow(dead_code)]
    pub fn print_ansi_stdout(&mut self, message: &[u8]) {
        if self.needs_clear {
            self.err_erase_line();
        }
        #[cfg(windows)]
    ...
    pub fn print_json<T: serde::ser::Serialize>(&mut self, obj: &T) {
        ...
        let encoded = serde_json::to_string(&obj).expect("could not deserialize object to json.");
...
impl ShellOut {
    ...
    fn message_stderr(
        ...
        match *self {
            ShellOut::Stream { ref mut stderr, .. } => {
                stderr.reset().expect("could bot reset stderr.");
                stderr
                    .set_color(ColorSpec::new().set_bold(true).set_fg(Some(color)))
                    .expect("could not set stderr color");
                if justified {
                    write!(stderr, "{:>12}", status).expect("could not justify stderr stream.");
                } else {
                    write!(stderr, "{}", status).expect("could not write to stderr.");
                    stderr
                        .set_color(ColorSpec::new().set_bold(true))
                        .expect("could not set stderr color.");
                    write!(stderr, ":").expect("could not write to stderr.");
                }
                stderr.reset().expect("could not reset stderr.");
                match message {
                    Some(message) => {
                        writeln!(stderr, " {}", message).expect("could not write to stderr.")
                    }
                    None => write!(stderr, " ").expect("could not write to stderr."),
                };
            }
            ShellOut::Write(ref mut w) => {
                if justified {
                    write!(w, "{:>12}", status).expect("could not write to stream.");
                } else {
                    write!(w, "{}:", status).expect("could not write to stream.");
                }
                match message {
                    Some(message) => {
                        writeln!(w, " {}", message).expect("could not write to stream.")
                    }
                    None => write!(w, " ").expect("could not write to stream."),
                };
            }
        }
    }
    ...
    /// Write a styled fragment
    fn write_stdout(&mut self, fragment: impl fmt::Display, color: &ColorSpec) {
        match *self {
            ShellOut::Stream { ref mut stdout, .. } => {
                ...
                write!(stdout, "{}", fragment).expect("could not print to stdout.");
                stdout.reset().expect("could not reset stdout");
            }
            ShellOut::Write(ref mut w) => {
                write!(w, "{}", fragment).expect("could not print to stdout.")
            }
        }
    }

    /// Write a styled fragment
    fn write_stderr(&mut self, fragment: impl fmt::Display, color: &ColorSpec) {
        match *self {
            ShellOut::Stream { ref mut stderr, .. } => {
                stderr.reset().expect("could not reset stdout");
                stderr
                    .set_color(&color)
                    .expect("could not set stdout color");
                write!(stderr, "{}", fragment).expect("could not print to stdout.");
                stderr.reset().expect("could not reset stdout");
            }
            ShellOut::Write(ref mut w) => {
                write!(w, "{}", fragment).expect("could not print to stdout.");
